...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = 
            let n1 = Node ("E", Empty, Empty)
            let n2 = Node ("D", Empty,n1)
            let n3 = Node ("C", Empty, n2)
            let n4 = Node ("B", Empty, Empty)
            Node ("A", n4, n3) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
